...
from app.database import init_db, list_strategy_registry, validation_coverage, save_research_org_report, save_agent_run_artifact, save_latest_artifact, latest_artifact
...
AGENT_ROLE_SUMMARY = {
    ...
    'external_mover_validation': ('External Mover Validation', '외부 급등/거래량 상위 N개를 일배치로 수집·backfill·paper 검증'),
    ...
    'next_trade_issue_context': ('Next Trade Issue Context', '다음 추천/검증 배치용 선제 이슈 컨텍스트'),
    ...
    'investor_flow_outcome_evaluator': ('Investor Flow Outcome Evaluator', '외국인/기관 seed 일별 시계열의 benchmark-relative 사후성과를 평가'),
    ...
    'research_org_orchestrator': ('Research Agenda Orchestrator', '수익률 개선 병목을 director 소유 queue로 라우팅하는 메타 조정자'),
    ...
    'data_steward': ('Data Steward', '데이터 도메인 총괄: universe/가격/공시/seed 데이터의 freshness와 품질 repair queue를 관리'),
    'market_context_director': ('Market Context Director', '시장맥락 도메인 총괄: issue/regime/route/context 신호의 역할 분리와 추천/검증 연결을 관리'),
    'strategy_director': ('Strategy Director', '전략 도메인 총괄: 생성/검증/수명주기/audit의 역할 적합성과 중복 작업을 평가'),
    'fund_director': ('Fund Director', 'Fund 도메인 총괄: paper fund 리그/registry/성과/risk/consensus 품질을 평가'),
    'recommendation_desk_lead': ('Recommendation Desk Lead', '추천 도메인 총괄: 추천/critic/risk/committee/검증 피드백의 역할 분리를 평가'),
    'suborg_summary': ('Sub-Organization Summary', '부서별 compact contract와 research queue를 생성해 UI/agent가 큰 artifact 없이 상태를 읽도록 함'),
    'governance_director': ('Governance Director', '거버넌스 도메인 총괄: integrity/org review/experiment workflow/patch proposal ownership을 관리'),
    'executive_director': ('Executive Organization Director', '전체 조직 총괄: 도메인 총괄과 suborg 상태를 관리하고 escalation/next-cycle 우선순위를 정리'),
}
...
def read_json(path: str) -> dict:
    try:
        db_payload = latest_artifact(artifact_key_from_path(path))
...
def main():
    ...
    add('next_trade_issue_context',['tools/agents/next_trade_issue_context_agent.py'],['/tmp/next_trade_issue_context_latest.json'])
    add('market_context_director',['tools/agents/market_context_director_agent.py'],['/tmp/market_context_director_latest.json'])
    ...
    add('data_steward',['tools/agents/data_steward_agent.py'],['/tmp/data_steward_latest.json'])
    ...
    add('fund_director',['tools/agents/fund_director_agent.py'],['/tmp/fund_director_latest.json'])
    ...
    add('investor_flow_outcome_evaluator',['tools/agents/investor_flow_outcome_evaluator_agent.py','--horizon-days','5'],['/tmp/investor_flow_outcome_evaluator_latest.json'])
    ...
    add('recommendation_desk_lead',['tools/agents/recommendation_desk_lead_agent.py'],['/tmp/recommendation_desk_lead_latest.json'])
    if logic_csv:
    ...
    add('strategy_director',['tools/agents/strategy_director_agent.py'],['/tmp/strategy_director_latest.json'])
    ...
    add('governance_director',['tools/agents/governance_director_agent.py'],['/tmp/governance_director_latest.json'])
    ...
    add('executive_director',['tools/agents/executive_director_agent.py'],['/tmp/executive_director_latest.json'])
    ...
    investor_flow_outcome_eval=read_json('/tmp/investor_flow_outcome_evaluator_latest.json')
    ...
    next_trade_issue_context=read_json('/tmp/next_trade_issue_context_latest.json')
    ...
    executive_director=read_json('/tmp/executive_director_latest.json')
    ...
    for fund_org_path in (Path('/tmp/fund_org_summary_latest.json'), Path('/tmp/fund_suborg_summary_latest.json'), ROOT / 'static' / 'fund_org_summary_latest.json', ROOT / 'static' / 'fund_suborg_summary_latest.json'):
        try:
