...
def build_context_goal_artifacts(report: dict, recs: dict, audit: dict, steps: list[dict], status: str, summary: str, next_actions: list[str]) -> tuple[dict, dict, dict, dict]:
    ...
    local_llm_delegation = {
        ...
        'output_contract': {
            'format': 'json',
            'required_keys': ['task_id', 'status', 'findings', 'evidence_refs', 'next_action'],
            'status_values': ['routine', 'needs_human_review', 'proposal_only', 'blocked'],
            'max_findings': 5,
        },
        'delegation_queue': [
            {'task_id': 'pipeline_health_review', 'purpose': 'Check failures, skips, drift, and degraded agents from compact status only.', 'inputs': ['/tmp/research_pipeline_status.json', '/tmp/context_goal_latest.json'], 'notify_if': ['failure_count > 0', 'consecutive_failures > 0', 'unexpected degraded required agent']},
            {'task_id': 'recommendation_drift_review', 'purpose': 'Review new/removed symbols, bucket moves, and top-card entry-plan sanity without opening full recommendations.', 'inputs': ['/tmp/recommendations_status_latest.json'], 'notify_if': ['new_symbols non-empty', 'large bucket churn', 'approved/research_watch inconsistency', 'entry plan missing on top items']},
            {'task_id': 'audit_quality_review', 'purpose': 'Judge whether current strategy trust labels, role profiles, and fund-routing evidence are weak, improving, or anomalous from compact audit status; prioritize strategy_trust_improvement_plan actions.', 'inputs': ['/tmp/audit_status_latest.json'], 'notify_if': ['quality_flags improve materially', 'best logic changes', 'positive excess appears', 'tail-risk anomaly appears', 'top trust-label action changes']},
            {'task_id': 'org_improvement_review', 'purpose': 'Propose one concrete low-risk code/docs improvement from compact pipeline summaries.', 'inputs': ['/tmp/context_goal_latest.json'], 'notify_if': ['non-trivial proposal with clear file target and verification gate']},
        ],
        'routing': {
...
def build_fund_org_summary_artifact(report: dict) -> dict:
    ...
    component_effectiveness = [
        ...
        },
        {
            ...
            'recommendation': 'Effective as a guardrail, but should add concentration/exposure caps when live holdings mature.',
        },
        {
