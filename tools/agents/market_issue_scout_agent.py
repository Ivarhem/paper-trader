...
def main():
 ...
 ap.add_argument('--output',default='/tmp/market_issue_scout_latest.json'); ap.add_argument('--limit-symbols',type=int,default=700); args=ap.parse_args(); init_db()
 conn=sqlite3.connect(get_settings().database_path); conn.row_factory=sqlite3.Row
 u=conn.execute("SELECT symbol,payload_json FROM universe_members WHERE status IN ('active','watch','candidate') ORDER BY score DESC LIMIT ?",(args.limit_symbols,)).fetchall()
 symbols=[]; names={}
 for row in u:
  payload=safe_json(row['payload_json']); symbols.append(row['symbol']); names[row['symbol']]=payload.get('name') or row['symbol']
 ...
 metrics=[m for s in symbols for m in [metric(conn,s,names.get(s))] if m]
 conn.close()
 ...
 for m in hot:
  ...
  key=(m['market'], toks[0] if toks else 'unlabeled_momentum')
  groups[key].append(m)
 issues=[]
 for (market,label),members in groups.items():
  if len(members)<2 and label=='unlabeled_momentum': continue
  ...
  label_txt=(label+' 강세') if label!='unlabeled_momentum' else f'{market} 모멘텀 클러스터'
  issues.append({'issue_id':f'{market.lower()}_{label}_{datetime.now(timezone.utc).strftime("%Y%m%d_%H%M")}'.replace(' ','_'),'type':'dynamic_market_issue_cluster','market':market,'label':label_txt,'theme_hint':label,'impact_score':round(score,2),'risk':risk,'expected_impact':'positive','member_count':len(members),'affected_symbols':[x['symbol'] for x in members],'members':members[:12],'avg_1d_return_pct':avg1,'avg_5d_return_pct':avg5,'avg_volume_spike':vsp,'breadth_positive_pct':breadth,'narrative':f"가격/거래량 기반 동적 감지: {label_txt}, 평균 1D {avg1}%, 5D {avg5}%, 거래량 {vsp}배.",'confidence':round(min(0.95,0.45+len(members)*0.06+(0.12 if label!='unlabeled_momentum' else 0)),2),'recommendation_policy':'watch_boost_only' if risk!='normal' else 'context_boost_allowed'})
 issues=sorted(issues,key=lambda x:(x['impact_score'],x['member_count']),reverse=True)[:12]
 packet={'run_at':utc_now(),'mode':'dynamic_market_issue_scout','real_trading':False,'issues':issues,'summary':{'issue_count':len(issues),'top_issues':[{'label':x['label'],'market':x['market'],'impact_score':x['impact_score'],'risk':x['risk'],'member_count':x['member_count']} for x in issues[:5]]}}
 attach_contract(packet,'market_issue_scout_agent',status='ok',outputs={'issue_count':len(issues),'top_issues':packet['summary']['top_issues']},metrics={'scanned_symbols':len(symbols),'metric_symbols':len(metrics),'hot_symbols':len(hot)},warnings=[],next_actions=['Connect high-confidence clusters to Market Regime Gate and validation priority.'])
